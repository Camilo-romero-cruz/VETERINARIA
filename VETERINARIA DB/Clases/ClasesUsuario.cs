    ...
    public class ClasesUsuario
    {
        ...
        public bool Login(string NombreUsuario, string contraseña)
        {
            try
            {
                ...
                var usuario = _context.Usuarios.FirstOrDefault(u => u.NombreUsuario == NombreUsuario && u.Contraseña == hashedPassword);
                return usuario != null;
            }
            catch (Exception)
            {
                // En un entorno real, podrías loguear la excepción aquí si lo deseas.
                return false;
            } 
